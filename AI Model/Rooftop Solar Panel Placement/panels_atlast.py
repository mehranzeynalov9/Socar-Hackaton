...
parser = argparse.ArgumentParser(description='Sekil ucun arqument')
    ...
    for _ in range(panels_series - 2):
        for col in range(0, cols, l + 1):
            for row in range(0, rows, w + 1):
                ...
                solar_patch = high_reso[row:row + (w + 1) * pw + 1, col:col + ((l * pl) + 3)] # Rectangular Region of interest for solar panel area
